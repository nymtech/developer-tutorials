...
function decodeStringifiedMessage(message : string){
    ...
    /*
        We need to decode the message that we have recieved from our client, where it was JSON.stringify'd before it was sent to our service provider.
    */
    ...
    */
    ...
    /*
        Make a new string value which we can pass into the UI (Recieved Message Data section).
    */
...
/*
    Function that renders the contents of our recievedMessageData array in the Recieved Message Data section of our UI.
*/
...
function handleReceivedMessage(message : MixnetMessage) {
    ...
    //Make the message that we recieve appear in the Activity Log.
    displayJsonResponseWithoutReply(text)

    //Remove slashes , convert message back into json object and add it to our recieved messages data section.
    recievedMessageData.push(decodeStringifiedMessage(text));

    //Re-render our UI to display our updated recieved message data.
    renderMessageList();
}
...
/*
    Function that takes a the incomming message (as sting) as a parameter and diplays it as a new entry in the Activity Log.
*/
